...
class DataUnderstanding():
    def verificando_as_dimensões_do_dataframe(self, dataframe: pd.DataFrame) -> None:
        dataframe = dataframe.shape
        print(f'Linhas: {dataframe[0]} \nColunas: {dataframe[1]}')

    def descrição(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        dataframe = dataframe.describe().round(decimals=2)
        return dataframe

    def verificando_tipos(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        dataframe = (
            dataframe.dtypes
            .to_frame('Tipos')
        )
    ...
    def verificando_valores_nulos(self, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    def verificando_valores_duplicados(self, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    def frequencia_da_repetição_dos_valores(self, dataframe: pd.DataFrame, coluna: str) -> pd.DataFrame:
        dataframe = dataframe[coluna].value_counts().reset_index()
        return dataframe

    def verificando_correlação(self, dataframe: pd.DataFrame) -> pd.DataFrame:
